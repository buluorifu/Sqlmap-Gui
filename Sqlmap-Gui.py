...
class MyWindow(QWidget, Ui_Form):
    ...
    def initial(self):
        ...
        self.center_window()  # 调用自定义的居中方法

        with open(f"./config/style.qss", "r", encoding="utf-8") as f:
            self.setStyleSheet(f.read())

        if os.path.exists('./config/file_address.yaml'):
            with open('./config/file_address.yaml', 'r', encoding='utf-8') as file:
    ...
    def sqlmap_file(self):
        ...
        if fileName:
            ...
            if os.path.exists('./config/file_address.yaml'):
                ...
                with open('./config/file_address.yaml', 'w', encoding='utf-8') as file:
